...
class LoadImages_infer(Dataset):  # for inference
    ...
    def __getitem__(self, index):
        ...
        # img0 = cv2.cvtColor(cv2.imread(path, 0), cv2.COLOR_GRAY2BGR) # grayscale to BGR

        assert img0 is not None, 'Image Not Found ' + path
        h0, w0 = img0.shape[:2]  # orig hw

        # Padded resize
        img, ratio, pad = letterbox(img0, self.img_size, stride=2048)
        ...
        return torch.from_numpy(img), torch.empty(0, 6), path, shapes
...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=8, image_weights=False, quad=False, prefix='', infer=False):
    # Make sure only the first process in DDP process the dataset first, and the following others can use the cache
    with torch_distributed_zero_first(rank):
        if infer:
            dataset = LoadImages_infer(path.replace('val.txt', 'images/val/'), img_size=imgsz, stride=stride)
        else:
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        # img = cv2.cvtColor(cv2.imread(path, 0), cv2.COLOR_GRAY2BGR) # grayscale to BGR
